...
The [[Screen]] is useful for for controlling the shape and behavior of your screen.
...
The above snippet has the effect of setting the logical resolution to 800x600 and the viewport css pixels to 800x600.

## Understanding Viewport & Resolution

Excalibur has a screen abstraction for dealing with the [[Screen.viewport]] size, and [[Screen.resolution]]. It also handles HiDPI scaling, the browser fullscreen API, and translation of screen coordinates into game world coordinates.

The [[Screen.viewport]] represents the size of the window in the game as the size of CSS pixels on the screen. Another way to think about the viewport is that it represents the actual size of the "window" in the game as shown on the computer screen.

The [[Screen.resolution]] represents the number of logical pixels stretched across the viewport.

For example, a screen with a resolution of 100x100 and a viewport of 1000x1000, stretches 100 logical game pixels across 1000 pixels of the screen.

The screen resolution and viewport can be set and accessed in the engine constructor.
